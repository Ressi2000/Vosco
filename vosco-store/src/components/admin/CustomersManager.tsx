...
export default function CustomersManager({ initialCustomers }: { initialCustomers: Customer[] }) {
  ...
  const handleSave = async () => {
    ...
    if (editingId) {
      const { data } = await supabase.from('customers').update(payload).eq('id', editingId).select().single()
              ...
              <AnimatePresence>
                {filtered.map(c => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4 text-[#B0B8C1] text-sm max-w-[180px] truncate">{c.address || '—'}</td>
